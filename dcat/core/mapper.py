...
DATASET_KNOWN_KEYS = ['dc:identifier', 'dc:title', 'dc:description',
                      ...
                      'dcat:theme', 'dc:subject', 'dcat:granularity',
                      'opmv:wasGeneratedBy', 'dc:created']

# TODO: Implement a language hint parameter to select appropriate language from 
...
def dcat_to_ckan(graph, dataset):
    ...
    extras['date_created'] = first(_query("""?dataset dc:created ?out"""))
    ...
    extras['publisher'] = first_or_list(_query("""{?dataset dc:publisher ?out FILTER
        isLiteral(?out)} UNION {?dataset dc:publisher ?pub . ?pub rdfs:label ?out}"""))
